...
const PROJECT_MODE_LOCAL = "local";
const PROJECT_MODE_PUBLISHED = "published";
const PROJECT_MODE_CLOUD = "cloud";
...
const dom = {
  ...
  copyProjectLinkBtn: document.querySelector("#copyProjectLinkBtn"),
  copyItemLinkBtn: document.querySelector("#copyItemLinkBtn"),
  copyToLibraryBtn: document.querySelector("#copyToLibraryBtn"),
  ...
  gridCardFooterFieldSelect: document.querySelector("#gridCardFooterFieldSelect"),
...
const state = {
  ...
  projectMode: PROJECT_MODE_LOCAL,
  publishScope: localStorage.getItem(PUBLISH_SCOPE_STORAGE_KEY) === PUBLISH_SCOPE_PROJECT ? PUBLISH_SCOPE_PROJECT : PUBLISH_SCOPE_INVENTORY,
  publishedSlug: "",
  publishedBasePath: "",
...
class IndexedArtifactStore {
  ...
  async importProjectRecords(records) {
    await this.tx(["artifacts", "images", "fields"], "readwrite", ({ artifacts, images, fields }) => {
      records.fields.forEach((field) => fields.put(field));
      records.artifacts.forEach((artifact) => artifacts.put(artifact));
      records.images.forEach((image) => images.put(image));
    });
  }
...
function isPublishedMode() {
  return state.projectMode === PROJECT_MODE_PUBLISHED || state.projectMode === PROJECT_MODE_CLOUD;
}

function getPublishedAccessMode(manifest = state.publishedManifest) {
  return manifest?.accessMode === SHARE_ACCESS_EDITABLE || manifest?.project?.accessMode === SHARE_ACCESS_EDITABLE
    ? SHARE_ACCESS_EDITABLE
    : SHARE_ACCESS_READ_ONLY;
}

function isReadOnlyMode() {
  if (state.projectMode === PROJECT_MODE_CLOUD) return getPublishedAccessMode() !== SHARE_ACCESS_EDITABLE;
  return state.projectMode === PROJECT_MODE_PUBLISHED;
}

function ensureEditable(message) {
  if (!isReadOnlyMode()) return true;
  window.alert(message || (state.language === "en"
    ? "This published project is read-only. Use Copy to My Library to edit your own copy."
    : "Published Project 为只读项目。请先 Copy to My Library，再编辑本地副本。"));
  return false;
}

function readFieldVisibilityOverrides() {
  try {
    const parsed = JSON.parse(localStorage.getItem(READONLY_FIELD_VISIBILITY_STORAGE_KEY) || "{}");
...
function saveFieldVisibilityOverrides() {
  localStorage.setItem(READONLY_FIELD_VISIBILITY_STORAGE_KEY, JSON.stringify(state.fieldVisibilityOverrides || {}));
}

function getFieldVisibilityOverrideKey(field) {
  const projectKey = state.publishedSlug || state.project?.id || state.project?.title || "local";
  return `${projectKey}:${field.id}`;
}

function getFieldVisibleInList(field) {
  if (!field) return false;
  const overrideKey = getFieldVisibilityOverrideKey(field);
  if (isReadOnlyMode() && Object.prototype.hasOwnProperty.call(state.fieldVisibilityOverrides, overrideKey)) {
    return Boolean(state.fieldVisibilityOverrides[overrideKey]);
  }
  return Boolean(field.visibleInList);
}

async function setFieldVisibleInList(field, visible) {
  if (!field) return;
  if (isReadOnlyMode()) {
    state.fieldVisibilityOverrides[getFieldVisibilityOverrideKey(field)] = Boolean(visible);
    saveFieldVisibilityOverrides();
    renderFieldList();
    renderCollection();
    return;
  }
...
function openArtifactFromNetwork(artifactId) {
  ...
  if (!isPublishedMode()) ensureStorageState();
  state.activeArtifactId = artifact.id;
  state.selectedIds = new Set([artifact.id]);
  state.activeImageId = artifact.imageIds?.[0] || null;
  window.location.hash = getArtifactRouteHash(artifact.id);
  render();
}

function getArtifactRouteHash(artifactId) {
  if (state.projectMode === PROJECT_MODE_CLOUD && state.cloudManifestUrl) {
    return `#/cloud/${encodeURIComponent(state.publishedSlug)}/items/${encodeURIComponent(artifactId)}`;
  }
  if (isPublishedMode()) {
    return `#/project/${encodeURIComponent(state.publishedSlug)}/items/${encodeURIComponent(artifactId)}`;
  }
...
function applyLanguage() {
  ...
  if (dom.exportProjectBtn) dom.exportProjectBtn.textContent = state.language === "en" ? "Export" : "导出";
  if (dom.importProjectBtn) dom.importProjectBtn.textContent = state.language === "en" ? "Import" : "导入";
  if (dom.copyProjectLinkBtn) dom.copyProjectLinkBtn.textContent = state.language === "en" ? "Copy Project Link" : "复制项目链接";
  if (dom.copyItemLinkBtn) dom.copyItemLinkBtn.textContent = state.language === "en" ? "Copy Item Link" : "复制条目链接";
  if (dom.copyToLibraryBtn) dom.copyToLibraryBtn.textContent = state.language === "en" ? "Copy to My Library" : "复制到我的本地库";
  if (dom.exportProjectBtn) {
    dom.exportProjectBtn.textContent = "Export";
    dom.exportProjectBtn.title = "Download a file or create a share link.";
  }
...
function showWorkspaceMode(mode, updateHash = true) {
  ...
  if (isNetwork && dom.networkFrame) {
    ...
    if (!dom.networkFrame.src || !dom.networkFrame.src.endsWith(desiredSrc)) {
      dom.networkFrame.src = desiredSrc;
    }
  }
  ...
  if (isNetwork) {
    if (state.projectMode === PROJECT_MODE_CLOUD && state.cloudManifestUrl) {
      window.location.hash = `#/cloud/${encodeURIComponent(state.publishedSlug)}/network`;
    } else {
      window.location.hash = isPublishedMode()
        ? `#/project/${encodeURIComponent(state.publishedSlug)}/network`
        : "network";
    }
...
function getNetworkFrameSrc() {
  const base = dom.networkFrame?.dataset.src || "network.html?embedded=1";
  if (!isPublishedMode()) return base;
  const separator = base.includes("?") ? "&" : "?";
  if (state.projectMode === PROJECT_MODE_CLOUD && state.cloudManifestUrl) {
    return `${base}${separator}manifest=${encodeURIComponent(state.cloudManifestUrl)}`;
  }
  return `${base}${separator}project=${encodeURIComponent(state.publishedSlug)}`;
}

function notifyNetworkInventoryChanged() {
  localStorage.setItem("easy-network-inventory-updated-at", String(Date.now()));
  dom.networkFrame?.contentWindow?.postMessage({
    type: "easy-network-inventory-updated",
    projectMode: state.projectMode,
    projectSlug: state.publishedSlug
...
function bindEvents() {
  ...
  dom.exportProjectBtn?.addEventListener("click", openExportDialog);
  dom.importProjectBtn?.addEventListener("click", openImportDialog);
  dom.importProjectInput?.addEventListener("change", async (event) => {
    const file = event.target.files?.[0];
    event.target.value = "";
    if (file) await importProjectFile(file);
  });
  dom.copyProjectLinkBtn?.addEventListener("click", copyPublishedProjectLink);
  dom.copyItemLinkBtn?.addEventListener("click", copyPublishedItemLink);
  dom.copyToLibraryBtn?.addEventListener("click", copyPublishedProjectToLibrary);
  ...
  dom.gridCardFooterFieldSelect?.addEventListener("change", () => {
    state.gridCardFooterFieldId = dom.gridCardFooterFieldSelect.value;
    localStorage.setItem(GRID_CARD_FOOTER_FIELD_STORAGE_KEY, state.gridCardFooterFieldId);
    renderCollection();
  });
...
function setViewMode(mode) {
  ...
  dom.gridCardFooterFieldSelect?.classList.toggle("hidden", mode !== "grid");
  renderCollection();
}
...
function render() {
  ...
  dom.projectSummary.textContent = isPublishedMode()
    ? `Published Project - Read Only - ${visibleCount} ${tr("items")}`
    : `Inventory - ${state.activeStorageName} - ${visibleCount} ${tr("items")}`;
...
function applyProjectModeUi() {
  document.body.dataset.projectMode = state.projectMode;
  const readOnly = isReadOnlyMode();
  const hasArtifact = Boolean(getActiveArtifact());
  const status = state.publishedError
    ? `Published Project - ${state.publishedError}`
    : readOnly
      ? `Published Project - Read Only - ${state.project?.title || state.publishedSlug}`
      : `Local Project - ${state.activeStorageName}`;
  if (dom.projectSummary) {
    dom.projectSummary.hidden = false;
    dom.projectSummary.textContent = status;
    dom.projectSummary.title = status;
  }
  [
    dom.undoBtn,
    dom.newProjectBtn,
    dom.createEmptyBtn,
    dom.libraryCreateEmptyBtn,
    dom.addFieldBtn,
    dom.addCustomFieldBtn,
    dom.expandedAddFieldBtn,
    dom.expandedInlineAddFieldBtn,
    dom.deleteImageBtn,
    dom.saveImageAdjustmentsBtn
  ].forEach((button) => {
    if (button) button.disabled = readOnly;
  });
  document.querySelectorAll("label[for='uploadCreateInput'], label[for='addImagesInput']").forEach((label) => {
    label.classList.toggle("disabled", readOnly);
    label.setAttribute("aria-disabled", String(readOnly));
  });
  if (dom.importProjectBtn) dom.importProjectBtn.classList.toggle("disabled", false);
  if (dom.exportProjectBtn) dom.exportProjectBtn.hidden = false;
  if (dom.importProjectBtn) dom.importProjectBtn.hidden = false;
  if (dom.copyProjectLinkBtn) dom.copyProjectLinkBtn.hidden = !readOnly;
  if (dom.copyItemLinkBtn) {
    dom.copyItemLinkBtn.hidden = !readOnly;
    dom.copyItemLinkBtn.disabled = !hasArtifact;
  }
  if (dom.copyToLibraryBtn) dom.copyToLibraryBtn.hidden = !readOnly || !state.publishedManifest;
}
...
function getGridCardFieldOptions(storageName = currentWriteStorageName()) {
  return fieldsForStorage(storageName, { includeCustom: true })
    .filter((field) => field.id !== "system-image-path");
}
...
function getAutoGridCardField(artifacts, options = {}) {
  ...
  return getGridCardFieldOptions()
    ...
    .map((field, index) => ({
      ...
      visibleRank: getFieldVisibleInList(field) ? 1 : 0
    }))
...
function getGridCardFieldById(fieldId, artifacts, options = {}) {
  if (fieldId === GRID_CARD_FIELD_NONE) return null;
  const fields = getGridCardFieldOptions();
  if (fieldId) {
    return fields.find((field) => field.id === fieldId) || null;
  }
  return getAutoGridCardField(artifacts, options);
}

function getGridCardFields(artifacts) {
  const primary = getGridCardFieldById(state.gridCardFieldId, artifacts, {
    excludeIds: ["system-id", "system-title", "system-image-path"]
  });
  const footer = getGridCardFieldById(state.gridCardFooterFieldId, artifacts, {
    excludeIds: ["system-image-path"]
  });
  return { primary, footer };
}

function renderGridCardSelect(select, artifacts, config) {
  if (!select) return;
  const isGrid = state.viewMode === "grid" && state.activeStorageName !== ALL_STORAGE_KEY;
  select.classList.toggle("hidden", !isGrid);
  select.disabled = !isGrid;
  if (!isGrid) return;
  const fields = getGridCardFieldOptions();
  const autoField = getAutoGridCardField(artifacts, { excludeIds: config.autoExcludeIds });
  const selectedValue = state[config.stateKey];
  const selectedIsValid = !selectedValue
    || selectedValue === GRID_CARD_FIELD_NONE
    || fields.some((field) => field.id === selectedValue);
  if (!selectedIsValid) {
    state[config.stateKey] = config.fallbackValue || "";
    localStorage.setItem(config.storageKey, state[config.stateKey]);
  }
  select.innerHTML = "";
  select.append(new Option(autoField ? `${config.autoLabel}: ${autoField.label}` : config.autoLabel, ""));
  select.append(new Option(config.noneLabel, GRID_CARD_FIELD_NONE));
  fields.forEach((field) => select.append(new Option(field.label, field.id)));
  select.value = state[config.stateKey];
  select.title = config.title;
}

function renderGridCardFieldSelect(artifacts = []) {
  renderGridCardSelect(dom.gridCardFieldSelect, artifacts, {
    stateKey: "gridCardFieldId",
    storageKey: GRID_CARD_FIELD_STORAGE_KEY,
    autoLabel: "Top auto",
    noneLabel: "No top field",
    title: "Grid card top field",
    autoExcludeIds: ["system-id", "system-title", "system-image-path"]
  });
  renderGridCardSelect(dom.gridCardFooterFieldSelect, artifacts, {
    stateKey: "gridCardFooterFieldId",
    storageKey: GRID_CARD_FOOTER_FIELD_STORAGE_KEY,
    fallbackValue: "system-id",
    autoLabel: "Bottom auto",
    noneLabel: "No bottom field",
    title: "Grid card bottom field",
    autoExcludeIds: ["system-image-path"]
  });
}
...
function createFieldChip(field) {
  ...
  const shownInList = getFieldVisibleInList(field);
  visible.className = `icon-button ${shownInList ? "active" : ""}`;
  visible.type = "button";
  visible.title = state.language === "zh" ? "在列表中显示" : "Show in list";
  visible.title = shownInList ? "Shown in list" : "Hidden from list";
  visible.setAttribute("aria-label", visible.title);
  visible.setAttribute("aria-pressed", String(shownInList));
  visible.textContent = shownInList ? "✓" : "";
  visible.addEventListener("click", async () => {
    await setFieldVisibleInList(field, !getFieldVisibleInList(field));
  });
  ...
  if (!field.isSystemField) {
    ...
    remove.textContent = "x";
    remove.disabled = isReadOnlyMode();
...
function renderCollection() {
  ...
  const gridCardFields = getGridCardFields(artifacts);
  ...
  if (state.publishedError) {
    ...
    dom.emptyState.querySelector("h3").textContent = "Published Project Error";
    dom.emptyState.querySelector("p").textContent = state.publishedError;
  }
  dom.gridView.innerHTML = "";
  dom.listView.innerHTML = "";
  artifacts.forEach((artifact) => dom.gridView.append(createArtifactCard(artifact, gridCardFields)));
...
function renderStorageOverview() {
  ...
  if (isList) {
    ...
    if (!isReadOnlyMode()) dom.gridView.append(createNewStorageCard());
  }
  syncTableScrollSlider();
}
...
function createArtifactCard(artifact, gridCardFields = {}) {
  const card = document.createElement("article");
  ...
  const gridCardField = gridCardFields.primary || null;
  const gridCardFooterField = gridCardFields.footer || null;
  ...
  const summaryLine = summaryValue
    ...
    : "";
  const footerValue = gridCardFooterField ? cleanCell(getFieldValue(artifact, gridCardFooterField)) : "";
  const footerLine = footerValue
    ? `<div class="artifact-meta-line" title="${escapeAttr(gridCardFooterField.label)}">${escapeHtml(footerValue)}</div>`
    : "";
  body.innerHTML = `
...
function openColumnMenu(event) {
  ...
  event.stopPropagation();
  dom.columnMenu.innerHTML = "";
  fieldsForStorage(currentWriteStorageName(), { includeCustom: true }).forEach((field) => {
    const label = document.createElement("label");
    const checkbox = document.createElement("input");
    checkbox.type = "checkbox";
    checkbox.checked = getFieldVisibleInList(field);
    checkbox.addEventListener("change", async () => {
      await setFieldVisibleInList(field, checkbox.checked);
      dom.columnMenu.classList.remove("hidden");
    });
...
function openArtifactMenu(event, artifactId) {
  ...
  if (isPublishedMode()) {
    menuItems.splice(5, 0, { label: "Copy Item Link", action: copyPublishedItemLink });
    menuItems.forEach((item) => {
      if ([copySelectedArtifacts, cutSelectedArtifacts, pasteArtifactsFromClipboard, deleteSelectedArtifacts].includes(item.action)) {
        item.disabled = true;
      }
    });
  }
...
function exportArtifactsCsv(artifacts) {
  if (!artifacts.length) return;
  const csv = createArtifactsCsvText(artifacts, state.fields);
...
function createArtifactsCsvText(artifacts, fields) {
  const headers = fields.map((field) => field.label);
  const rows = artifacts.map((artifact) => fields.map((field) => getFieldValue(artifact, field)));
...
function selectArtifact(id, event = {}) {
  ...
  const artifact = getActiveArtifact();
  state.activeImageId = artifact?.imageIds?.[0] || null;
  window.location.hash = getArtifactRouteHash(id);
  renderCollection();
  renderDetail();
  renderExpandedView();
}
...
function renderCustomFields(container, artifact, expanded) {
  ...
  custom.forEach((field) => {
    ...
    if (!isReadOnlyMode()) row.addEventListener("contextmenu", (event) => openFieldContextMenu(event, field));
    container.append(row);
  });
}
...
function getImagesForArtifact(artifact) {
  ...
  return ids.map((id) => state.images.find((image) => image.id === id)).filter(Boolean);
}
...
function slugifyProjectName(value) {
  const ascii = String(value || "easy-network-project")
    .normalize("NFKD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
...
function getPublishTitle(scope = state.publishScope, storageName = "") {
  if (scope === PUBLISH_SCOPE_PROJECT) return state.project?.title || "Easy Network Project";
  if (storageName) return storageName;
  return state.activeStorageName === ALL_STORAGE_KEY
    ? (state.storages[0] || state.project?.title || "Easy Network Inventory")
    : state.activeStorageName;
}

function getPublishStorageName(scope = state.publishScope, storageName = "") {
  if (scope === PUBLISH_SCOPE_PROJECT) return "";
  if (storageName) return storageName;
  return state.activeStorageName === ALL_STORAGE_KEY ? (state.storages[0] || "") : state.activeStorageName;
}

function setPublishScope(scope) {
  state.publishScope = scope === PUBLISH_SCOPE_PROJECT ? PUBLISH_SCOPE_PROJECT : PUBLISH_SCOPE_INVENTORY;
  localStorage.setItem(PUBLISH_SCOPE_STORAGE_KEY, state.publishScope);
}

function getPublishSlug(scope = state.publishScope, storageName = "") {
...
function getPublicBaseUrl() {
  return (localStorage.getItem(PUBLIC_BASE_URL_STORAGE_KEY) || DEFAULT_PUBLIC_BASE_URL).replace(/\/?$/, "/");
}

function savePublicBaseUrl(value) {
  const clean = String(value || DEFAULT_PUBLIC_BASE_URL).trim() || DEFAULT_PUBLIC_BASE_URL;
  localStorage.setItem(PUBLIC_BASE_URL_STORAGE_KEY, clean.replace(/\/?$/, "/"));
}
...
function getDefaultProjectShareUrl(slug = getCurrentProjectSlug()) {
  return `${getPublicBaseUrl()}#/project/${encodeURIComponent(slug)}`;
}

function getDefaultItemShareUrl(artifactId, slug = getCurrentProjectSlug()) {
  return `${getPublicBaseUrl()}#/project/${encodeURIComponent(slug)}/items/${encodeURIComponent(artifactId)}`;
}

function getPublishScopeLabel(scope = state.publishScope, storageName = "") {
  return scope === PUBLISH_SCOPE_PROJECT ? "Entire Project" : `Inventory: ${getPublishTitle(PUBLISH_SCOPE_INVENTORY, storageName)}`;
}

function sanitizePackagePathPart(value, fallback = "file") {
...
function getExportZipFilename(slug, suffix = "") {
  const safeSlug = slugifyProjectName(slug || "project");
  const safeSuffix = suffix ? slugifyProjectName(suffix) : "";
...
async function createProjectPackage(options = {}) {
  ...
  const projectArtifacts = storageName
    ? state.artifacts.filter((artifact) => getArtifactStorageName(artifact) === storageName)
    : state.artifacts;
  if (!projectArtifacts.length) throw new Error("There are no items to export.");
  const artifactIds = new Set(projectArtifacts.map((artifact) => artifact.id));
  const projectFields = storageName
    ? state.fields.filter((field) => field.isSystemField || fieldIsDefinedInStorage(field, storageName))
    : state.fields;
  const sourceImages = state.images
    .filter((image) => artifactIds.has(image.artifactId))
    .sort((a, b) => (a.artifactId || "").localeCompare(b.artifactId || "") || (a.sortOrder || 0) - (b.sortOrder || 0));
  ...
  const artifacts = projectArtifacts.map((artifact) => {
    const imageIds = [...(artifact.imageIds || [])].filter((id) => pathByImageId.has(id));
    const imagePaths = imageIds.map((id) => pathByImageId.get(id));
    return {
      ...artifact,
      storageName: storageName || getArtifactStorageName(artifact),
      metadata: { ...(artifact.metadata || {}), "Image Path": imagePaths.join("; ") },
      customFields: { ...(artifact.customFields || {}) },
      imageIds
    };
  });
  const manifest = {
    ...
    packageType: options.publishable ? "published" : "portable",
    projectVersion: options.projectVersion || "1.0",
    publishedAt: options.publishable ? new Date().toISOString().slice(0, 10) : "",
    slug,
    scope,
    accessMode: options.accessMode === SHARE_ACCESS_EDITABLE ? SHARE_ACCESS_EDITABLE : SHARE_ACCESS_READ_ONLY,
    project: {
      ...(state.project || {}),
      id: state.project?.id || "project-local-archive",
      title: getPublishTitle(scope, storageName) || state.project?.title || "Easy Network Project",
      storageName: storageName || "",
      sourceMode: state.projectMode,
      accessMode: options.accessMode === SHARE_ACCESS_EDITABLE ? SHARE_ACCESS_EDITABLE : SHARE_ACCESS_READ_ONLY
    },
    storages: storageName ? [storageName] : getAllStorageNames(),
    fields: projectFields.map((field) => ({ ...field })),
    artifacts,
    images,
    csvPath: "data/artifacts.csv",
    networkData: readNetworkPackageData()
  };
  const csvText = createArtifactsCsvText(artifacts, projectFields);
  return { manifest, imageEntries, csvText, slug };
}

function readNetworkPackageData() {
  const data = {};
  for (let index = 0; index < localStorage.length; index += 1) {
    const key = localStorage.key(index);
    if (key?.startsWith("easy-network-analysis")) data[key] = localStorage.getItem(key);
  }
  return data;
}

function normalizeProjectManifest(raw, basePath = "") {
  ...
  const images = (raw.images || []).map((image) => {
    const path = image.path || image.url || image.storageKey || "";
    const url = path && !/^https?:|^blob:|^data:/i.test(path) && baseUrl
  ...
  return {
    ...
    slug,
    basePath,
    storages: raw.storages?.length ? raw.storages : [raw.project?.storageName || raw.project?.title || slug],
    fields: (raw.fields || []).map((field) => ({ ...field })),
...
async function fetchPublishedProject(slug) {
  const safeSlug = encodeURIComponent(slug);
  const basePath = `projects/${safeSlug}/`;
  const response = await fetch(`${basePath}project.json`, { cache: "no-cache" });
  if (!response.ok) throw new Error(`Published project "${slug}" was not found.`);
  return normalizeProjectManifest(await response.json(), basePath);
}

async function exportPortableProject(options = {}) {
  try {
    const scope = options.scope === PUBLISH_SCOPE_PROJECT ? PUBLISH_SCOPE_PROJECT : (options.scope || state.publishScope);
    const storageName = Object.prototype.hasOwnProperty.call(options, "storageName")
      ? options.storageName
      : getPublishStorageName(scope);
    const bundle = await createProjectPackage({ publishable: false, scope, storageName });
    const blob = await createZipBlob([
      { path: "project.json", data: JSON.stringify(bundle.manifest, null, 2) },
      { path: "data/artifacts.csv", data: `\uFEFF${bundle.csvText}` },
      ...bundle.imageEntries.map((entry) => ({ path: entry.path, data: entry.blob }))
    ]);
    downloadBlob(blob, getExportZipFilename(bundle.slug));
  } catch (error) {
    window.alert(error?.message || String(error));
  }
}

async function exportPublishableProject(options = {}) {
  try {
    const scope = options.scope === PUBLISH_SCOPE_PROJECT ? PUBLISH_SCOPE_PROJECT : (options.scope || state.publishScope);
    const storageName = Object.prototype.hasOwnProperty.call(options, "storageName")
      ? options.storageName
      : getPublishStorageName(scope);
    const cleanSlug = getPublishSlug(scope, storageName);
    const bundle = await createProjectPackage({ publishable: true, slug: cleanSlug, scope, storageName });
    const root = `projects/${cleanSlug}/`;
    const blob = await createZipBlob([
      { path: `${root}project.json`, data: JSON.stringify(bundle.manifest, null, 2) },
      { path: `${root}data/artifacts.csv`, data: `\uFEFF${bundle.csvText}` },
      ...bundle.imageEntries.map((entry) => ({ path: `${root}${entry.path}`, data: entry.blob }))
    ]);
    downloadBlob(blob, getExportZipFilename(cleanSlug, "publishable"));
    const link = getDefaultProjectShareUrl(cleanSlug);
    await copyTextToClipboard(link);
    window.alert(`Publishable package exported.\n\nScope: ${getPublishScopeLabel(scope, storageName)}\nDefault link:\n${link}\n\nPlace the exported projects/${cleanSlug}/ folder in the public site repository for this link to work.`);
  } catch (error) {
    window.alert(error?.message || String(error));
  }
}

function getCloudProjectShareLink(slug, manifestUrl) {
  if (!slug || !manifestUrl) return "";
  return `${getPublicBaseUrl()}#/cloud/${encodeURIComponent(slug)}`;
}

function openExportDialog() {
  const storageNames = getAllStorageNames();
  const currentStorageName = getPublishStorageName(PUBLISH_SCOPE_INVENTORY);
  let selectedStorageName = storageNames.includes(currentStorageName) ? currentStorageName : (storageNames[0] || "");
  let selectedScope = state.publishScope === PUBLISH_SCOPE_PROJECT ? PUBLISH_SCOPE_PROJECT : PUBLISH_SCOPE_INVENTORY;

  const backdrop = document.createElement("div");
  backdrop.className = "dialog-backdrop";
  const dialog = document.createElement("div");
  dialog.className = "dialog cloud-settings-dialog";
  const header = document.createElement("div");
  header.className = "dialog-header";
  const title = document.createElement("h2");
  ...
  const hint = document.createElement("p");
  hint.className = "quiet-line";
  hint.textContent = "Choose what to export, then download a file or create a share link.";

  const scopeLabel = document.createElement("label");
  scopeLabel.className = "field-label";
  scopeLabel.textContent = "Export";
  const scopeSelect = document.createElement("select");
  scopeSelect.className = "input";
  scopeSelect.append(new Option("This inventory", PUBLISH_SCOPE_INVENTORY));
  scopeSelect.append(new Option("Whole project", PUBLISH_SCOPE_PROJECT));
  scopeSelect.value = selectedScope;

  const inventoryLabel = document.createElement("label");
  inventoryLabel.className = "field-label";
  inventoryLabel.textContent = "Inventory";
  const inventorySelect = document.createElement("select");
  inventorySelect.className = "input";
  storageNames.forEach((name) => inventorySelect.append(new Option(name, name)));
  inventorySelect.value = selectedStorageName;

  const accessLabel = document.createElement("label");
  accessLabel.className = "field-label";
  accessLabel.textContent = "Link permissions";
  const accessSelect = document.createElement("select");
  accessSelect.className = "input";
  accessSelect.append(new Option("Read-only", SHARE_ACCESS_READ_ONLY));
  ...
  localButton.className = "button primary";
  localButton.type = "button";
  localButton.textContent = "Download File";
  const cloudButton = document.createElement("button");
  cloudButton.className = "button primary";
  ...
  function syncExportDialog() {
    ...
    const isInventory = options.scope === PUBLISH_SCOPE_INVENTORY;
    inventoryLabel.hidden = !isInventory;
...
function blobToDataUrl(blob) {
  return new Promise((resolve, reject) => {
    ...
    reader.addEventListener("error", () => reject(reader.error || new Error("Could not read file data.")));
...
function showCopyTextDialog(text) {
  const backdrop = document.createElement("div");
  backdrop.className = "dialog-backdrop";
  const dialog = document.createElement("div");
  dialog.className = "dialog";
  ...
  const hint = document.createElement("p");
  hint.className = "quiet-line";
  hint.textContent = "Select the link and copy it manually.";
  dialog.append(header, hint, input);
  backdrop.append(dialog);
  const close = () => backdrop.remove();
  closeButton.addEventListener("click", close);
  backdrop.addEventListener("click", (event) => {
...
async function copyPublishedProjectLink() {
  const slug = state.publishedSlug || slugifyProjectName(state.activeStorageName);
  const url = state.projectMode === PROJECT_MODE_CLOUD && state.cloudManifestUrl
    ? getCloudProjectShareLink(slug, state.cloudManifestUrl)
    : getDefaultProjectShareUrl(slug);
  await copyTextToClipboard(url);
}

async function copyPublishedItemLink() {
  const artifact = getActiveArtifact();
  if (!artifact) return;
  const slug = state.publishedSlug || slugifyProjectName(state.activeStorageName);
  const url = state.projectMode === PROJECT_MODE_CLOUD && state.cloudManifestUrl
    ? `${getPublicBaseUrl()}#/cloud/${encodeURIComponent(slug)}/items/${encodeURIComponent(artifact.id)}`
    : getDefaultItemShareUrl(artifact.id, slug);
  await copyTextToClipboard(url);
}
...
function openImportDialog() {
  ...
  const hint = document.createElement("p");
  hint.className = "quiet-line";
  hint.textContent = "Import a local Easy Network ZIP, or paste a shared project link to copy it into this browser.";

  const linkLabel = document.createElement("label");
  linkLabel.className = "field-label";
  linkLabel.textContent = "Shared project link";
  const linkInput = document.createElement("textarea");
  linkInput.className = "input";
  linkInput.rows = 3;
  linkInput.placeholder = `${getPublicBaseUrl()}#/cloud/project-name`;
  ...
  fileButton.className = "button primary";
  fileButton.type = "button";
  fileButton.textContent = "Import Local File";
  const linkButton = document.createElement("button");
  linkButton.className = "button primary";
  linkButton.type = "button";
  linkButton.textContent = "Import Link";
  actions.append(fileButton, linkButton);
  ...
  fileButton.addEventListener("click", () => {
    close();
    dom.importProjectInput?.click();
  });
  linkButton.addEventListener("click", async () => {
    const link = linkInput.value.trim();
    if (!link) {
      feedback.textContent = "Paste a project link first.";
      return;
    }
    linkButton.disabled = true;
    linkButton.textContent = "Importing...";
    try {
      await importProjectLink(link);
      close();
    } catch (error) {
      feedback.textContent = error?.message || String(error);
  ...
  dialog.append(header, hint, linkLabel, linkInput, feedback, actions);
  backdrop.append(dialog);
  document.body.append(backdrop);
  linkInput.focus();
}

function getManifestUrlFromSharedLink(value) {
  const raw = String(value || "").trim();
  if (!raw) throw new Error("Project link is required.");
  const appUrl = new URL(raw, location.href);
  if (/\/project\.json(?:$|\?)/i.test(appUrl.pathname)) return appUrl.href;

  const hash = appUrl.hash || raw;
  const cloudMatch = hash.match(/^#\/cloud\/([^/?]+)(?:\/(?:items|network)\/?[^/?]*)?(?:\?(.+))?$/);
  if (cloudMatch) {
    const params = new URLSearchParams(cloudMatch[2] || "");
    const source = params.get("source");
    if (source) return new URL(source, appUrl.href).href;
    const fromConfig = getCloudManifestUrlFromConfig(decodeURIComponent(cloudMatch[1]));
    if (fromConfig) return fromConfig;
    throw new Error("Share link is missing its source URL.");
  }

  const publishedMatch = hash.match(/^#\/project\/([^/]+)(?:\/(?:items|network)\/?[^/]*)?$/);
  if (publishedMatch) {
    const cleanUrl = new URL(appUrl.href);
    cleanUrl.hash = "";
    cleanUrl.search = "";
    const base = new URL(".", cleanUrl.href);
    return new URL(`projects/${encodeURIComponent(decodeURIComponent(publishedMatch[1]))}/project.json`, base.href).href;
  }

  throw new Error("This does not look like an Easy Network project link.");
}

async function importProjectLink(link) {
  const manifestUrl = getManifestUrlFromSharedLink(link);
  const manifest = await fetchCloudProject(manifestUrl);
  manifest.images = await Promise.all(manifest.images.map(async (image) => ({
    ...image,
...
async function importProjectFile(file) {
  try {
    ...
    const manifestEntry = entries.find((entry) => /(^|\/)project\.json$/i.test(entry.path));
    if (!manifestEntry) throw new Error("project.json was not found in this package.");
    const manifest = normalizeProjectManifest(JSON.parse(await new Blob([manifestEntry.data]).text()));
    const filesByPath = new Map(entries.map((entry) => [entry.path.replace(/\\/g, "/"), entry]));
    const root = manifestEntry.path.replace(/project\.json$/i, "");
    manifest.images = manifest.images.map((image) => {
      const path = image.path || image.url || "";
      const entry = filesByPath.get(path) || filesByPath.get(`${root}${path}`);
      if (!entry) return image;
      return {
        ...image,
        blob: new Blob([entry.data], { type: image.mimeType || guessMimeType(image.filename || path) })
      };
    });
    await copyProjectManifestToLibrary(manifest);
...
async function copyPublishedProjectToLibrary() {
  if (!state.publishedManifest) return;
  try {
    const manifest = normalizeProjectManifest(state.publishedManifest, state.publishedBasePath);
    manifest.images = await Promise.all(manifest.images.map(async (image) => ({
      ...image,
      blob: await imageBlobForPackage(image)
    })));
...
async function copyProjectManifestToLibrary(manifest) {
  ...
  manifest.artifacts.forEach((artifact) => {
    ...
    if (usedArtifactIds.has(nextId)) {
      const prefix = sanitizePackagePathPart(nextId, "ITEM");
      let index = 1;
      do {
        index += 1;
        nextId = `${prefix}-COPY-${index}`;
      } while (usedArtifactIds.has(nextId));
    }
    usedArtifactIds.add(nextId);
    artifactIdMap.set(artifact.id, nextId);
  });
  const fieldIdMap = new Map(SYSTEM_FIELDS.map((field) => [field.id, field.id]));
  const usedFieldIds = new Set(localState.fields.map((field) => field.id));
  const fields = [];
  (manifest.fields || []).forEach((field) => {
    if (field.isSystemField) return;
    const nextId = usedFieldIds.has(field.id) ? createId("FIELD") : field.id;
    usedFieldIds.add(nextId);
    ...
    fields.push({
      ...
      linkedStorageName: field.linkedStorageName || "",
      storageNames: [importedStorage],
      artifactIds: (field.artifactIds || []).map((id) => artifactIdMap.get(id)).filter(Boolean),
  ...
  (manifest.images || []).forEach((image, index) => {
    const artifactId = artifactIdMap.get(image.artifactId);
    if (!artifactId || !image.blob) return;
    const imageId = createId("IMG");
    imageIdMap.set(image.id, imageId);
    images.push({
      ...stripImageBlob(image),
      id: imageId,
      artifactId,
      storageKey: `local/${artifactId}/${image.filename || imageId}`,
      blob: image.blob,
  ...
  const artifacts = (manifest.artifacts || []).map((artifact) => {
    const nextId = artifactIdMap.get(artifact.id);
    const imageIds = (artifact.imageIds || []).map((id) => imageIdMap.get(id)).filter(Boolean);
    const customFields = {};
    Object.entries(artifact.customFields || {}).forEach(([fieldId, value]) => {
      const nextFieldId = fieldIdMap.get(fieldId);
      if (nextFieldId) customFields[nextFieldId] = value;
    });
    return {
      ...artifact,
      id: nextId,
      projectId: "project-local-archive",
      storageName: importedStorage,
      title: artifact.metadata?.Title || artifact.title || nextId,
      metadata: { ...(artifact.metadata || {}), "Image Path": images.filter((image) => image.artifactId === nextId).map((image) => image.storageKey).join("; ") },
      customFields,
      imageIds,
      createdAt: now(),
      updatedAt: now(),
  ...
  state.storages = [...new Set([...getAllStorageNamesFromState(localState), importedStorage])];
  state.activeStorageName = importedStorage;
  ensureStorageState();
  await refreshState();
...
function getAllStorageNamesFromState(snapshot) {
  let saved = [];
  try {
    saved = JSON.parse(localStorage.getItem("easy-network-storages") || "[]");
  } catch (error) {
    saved = [];
  }
  const names = new Set(saved);
  (snapshot.artifacts || []).forEach((artifact) => names.add(getArtifactStorageName(artifact)));
  (snapshot.fields || []).forEach((field) => {
    if (field.storageName) names.add(field.storageName);
    (field.storageNames || []).forEach((name) => names.add(name));
  });
  names.delete("");
  names.delete(ALL_STORAGE_KEY);
...
async function createZipBlob(entries) {
  ...
  for (const entry of entries) {
    ...
    const dataBytes = entry.data instanceof Blob ? await blobToBytes(entry.data) : encoder.encode(String(entry.data ?? ""));
    ...
    parts.push(localBytes, nameBytes, dataBytes);
    ...
    centralParts.push(headerBytes, nameBytes);
    centralLength += headerBytes.length + nameBytes.length;
...
async function inflateRaw(bytes) {
  if (!("DecompressionStream" in window)) throw new Error("This browser cannot import compressed ZIP entries.");
...
async function readZipEntries(file) {
  ...
  while (offset < bytes.length - 4) {
    ...
    if (path && !path.endsWith("/")) entries.push({ path, data });
    offset = dataEnd;
  }
  if (!entries.length) throw new Error("No readable files were found in this ZIP.");
  return entries;
}

async function applyRoute() {
  const cloudMatch = window.location.hash.match(/^#\/cloud\/([^/?]+)(?:\/(items|network)\/?([^/?]+)?)?(?:\?(.+))?$/);
  if (cloudMatch) {
    const slug = decodeURIComponent(cloudMatch[1]);
    const section = cloudMatch[2] || "";
    const itemId = cloudMatch[3] ? decodeURIComponent(cloudMatch[3]) : "";
    const params = new URLSearchParams(cloudMatch[4] || "");
    await loadCloudRoute(slug, section, itemId, params.get("source") || "");
    return;
  }
  const publishedMatch = window.location.hash.match(/^#\/project\/([^/]+)(?:\/(items|network)\/?([^/]+)?)?$/);
  if (publishedMatch) {
    const slug = decodeURIComponent(publishedMatch[1]);
    const section = publishedMatch[2] || "";
    const itemId = publishedMatch[3] ? decodeURIComponent(publishedMatch[3]) : "";
    await loadPublishedRoute(slug, section, itemId);
    return;
  }
  if (isPublishedMode()) {
    state.projectMode = PROJECT_MODE_LOCAL;
...
async function loadCloudRoute(slug, section, itemId, sourceUrl) {
  try {
    const manifestUrl = sourceUrl || state.cloudManifestUrl || getCloudManifestUrlFromConfig(slug);
    if (!manifestUrl) throw new Error("Share link is missing its project source URL.");
    const manifest = state.projectMode === PROJECT_MODE_CLOUD && state.cloudManifestUrl === manifestUrl && state.publishedManifest
      ? state.publishedManifest
      : await fetchCloudProject(manifestUrl);
    state.projectMode = PROJECT_MODE_CLOUD;
    state.cloudManifestUrl = manifestUrl;
    state.publishedSlug = manifest.slug || slug;
    state.publishedBasePath = manifest.basePath;
    state.publishedManifest = manifest;
    state.publishedError = "";
    state.project = {
      id: `cloud-${state.publishedSlug}`,
      title: manifest.project?.title || manifest.project?.storageName || state.publishedSlug,
      description: manifest.project?.description || "",
      shareToken: state.publishedSlug
    };
    state.artifacts = manifest.artifacts;
    state.images = manifest.images;
    state.fields = manifest.fields.length ? manifest.fields : SYSTEM_FIELDS;
    state.storages = manifest.storages;
    state.activeStorageName = manifest.storages[0] || state.project.title;
    const target = itemId
      ? state.artifacts.find((artifact) => artifact.id === itemId)
      : state.artifacts[0];
    if (itemId && !target) state.publishedError = `Item "${itemId}" was not found in this cloud project.`;
    state.activeArtifactId = target?.id || null;
    state.selectedIds = target ? new Set([target.id]) : new Set();
    state.activeImageId = target?.imageIds?.[0] || null;
    if (section === "network") showWorkspaceMode("network", false);
    else showWorkspaceMode("inventory", false);
    render();
  } catch (error) {
    state.projectMode = PROJECT_MODE_CLOUD;
    state.publishedSlug = slug;
    state.cloudManifestUrl = sourceUrl || "";
    state.publishedError = error?.message || String(error);
    state.project = { id: `cloud-${slug}`, title: slug };
    state.artifacts = [];
    state.images = [];
    state.fields = SYSTEM_FIELDS;
    state.storages = [slug];
    state.activeStorageName = slug;
    showWorkspaceMode("inventory", false);
    render();
  }
}

async function fetchCloudProject(manifestUrl) {
  const response = await fetch(manifestUrl, { cache: "no-cache" });
  if (!response.ok) throw new Error(`Cloud project could not be loaded: ${response.status}`);
  const basePath = manifestUrl.replace(/[^/]*$/, "");
  return normalizeProjectManifest(await response.json(), basePath);
}

function getCloudManifestUrlFromConfig(slug) {
  const shareServiceBase = getShareServiceBaseUrl();
  const base = shareServiceBase || location.origin;
  return `${base}/shared-projects/${encodeURIComponent(slug)}/project.json`;
}

async function loadPublishedRoute(slug, section, itemId) {
  try {
    const manifest = state.projectMode === PROJECT_MODE_PUBLISHED && state.publishedSlug === slug && state.publishedManifest
      ? state.publishedManifest
      : await fetchPublishedProject(slug);
    state.projectMode = PROJECT_MODE_PUBLISHED;
    state.publishedSlug = manifest.slug || slug;
    state.publishedBasePath = manifest.basePath || `projects/${encodeURIComponent(slug)}/`;
    state.publishedManifest = manifest;
    state.publishedError = "";
    state.project = {
      id: `published-${state.publishedSlug}`,
      title: manifest.project?.title || manifest.project?.storageName || state.publishedSlug,
      description: manifest.project?.description || "",
      shareToken: state.publishedSlug
    };
    ...
    const target = itemId
      ...
      : state.artifacts[0];
    if (itemId && !target) {
      state.publishedError = `Item "${itemId}" was not found in this published project.`;
    }
    state.activeArtifactId = target?.id || null;
    state.selectedIds = target ? new Set([target.id]) : new Set();
    state.activeImageId = target?.imageIds?.[0] || null;
    ...
    state.publishedSlug = slug;
    state.publishedError = error?.message || String(error);
    state.project = { id: `published-${slug}`, title: slug };
    state.artifacts = [];
    state.images = [];
    state.fields = SYSTEM_FIELDS;
    state.storages = [slug];
